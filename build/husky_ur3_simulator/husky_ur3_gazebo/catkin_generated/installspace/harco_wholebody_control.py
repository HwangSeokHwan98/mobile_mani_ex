...
class ManipulatorController:
    ...
    def move_to_point(self, x, y, z):
        ...
        # Calculate relative position from initial pose
        relative_x = x - self.initial_pose.pose.pose.position.x
        relative_y = y - self.initial_pose.pose.pose.position.y
        relative_z = z - self.initial_pose.pose.pose.position.z

        # Example: calculate joint positions based on relative end-point position
        # These are example values, you need to calculate actual joint positions based on your manipulator kinematics
        joint_positions = self.calculate_joint_positions(relative_x, relative_y, relative_z)
...
def amcl_pose_callback(data):
    # Callback function to receive initial pose
    controller = ManipulatorController(data)
    # Example: Move to a relative point (0.3312, 0.0, 0.4034) from the initial pose
    controller.move_to_point(data.pose.pose.position.x + 0.3312, data.pose.pose.position.y, data.pose.pose.position.z + 0.4034)
